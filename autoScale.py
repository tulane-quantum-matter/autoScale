...
def rootBisection(myFunc, xMin, xMax, epsilon):
    ...
    while abs(xMax - xMin) > 2.0 * epsilon:
        ...
        iter += 1
        file.write(
            "%3d: [%10.9lf,%10.9lf] [%10.9lf,%10.9lf]\n"
            ...
            if line[0] != "#" and len(stuff) >= 2:
                ...
                for line2 in datFile:  # collect data from corresp file
                    ...
                    if len(stuff2) > 2 and stuff2[0] != "#":
                        pivSet.append(
                            myValue(
                                L, float(stuff2[0]), float(stuff2[1]), float(stuff2[2])
                            )
                        )
...
class myFunc(myRawData, myScaleAssumption):
    ...
    def selectSubset(self, pivVal):
        ...
        subSet = []
        for L in [L for L in list(self.dataSet.keys()) if L != pivVal.L]:
            ...
            for val in self.dataSet[L]:
                ...
                if sVal.x <= sPiv.x and (minVal == None or sVal.x >= minVal.x):
                    minVal = sVal
    ...
    def llsFit(self, pivX, subSet):
        ...
        A, B = solve2x2(K, Kx, Kx, Kxx, Ky, Kxy)
        Y = A + B * pivX
        fac = K * Kxx - Kx * Kx
...
def errorAnalysis(f):
    ...
    for parId in range(len(optFlag)):
        ...
        if optFlag[parId] == 1:
            ...
            def dumFunc(val):
                scalePar[parId] = val
                return f.scaleData(scalePar) - (bestS + 1.0)
        ...
        # if scaling parameter was not optimized return error 0.
            ...
            parErr[parId] = [scalePar[parId], 0.0, 0.0]
...
def main():
    ...
    while arg < argMax:
        ...
        if sys.argv[arg] == "-f":
            ...
            if arg < argMax and os.path.exists(sys.argv[arg]):
                ...
                print(
                    "ERROR: option -f requires datafile. %s does not exist"
                    % sys.argv[arg]
                )
                sys.exit(1)
    ...
    if getError:
        ...
        for parId, values in parErr.items():
            outFile.write(
                "%4s = %lf %lf %lf\n"
    ...
    sys.stderr.write("Optimization took %d iterations\n" % nIter)
